...
public class TestJsonWorkflowJob extends TestCase {
    ...
    public void testList() throws Exception {
        List<JsonWorkflowJob> nodes = Arrays.asList(createWorkflow(), createWorkflow());
        JSONArray array = JsonWorkflowJob.toJSONArray(nodes);
        StringWriter sw = new StringWriter();
        array.writeJSONString(sw);
        sw.close();
        array = (JSONArray) JSONValue.parse(new StringReader(sw.toString()));
        List<WorkflowJob> readWorkflows = JsonWorkflowJob.fromJSONArray(array);
        assertEquals(2, readWorkflows.size());
    }
