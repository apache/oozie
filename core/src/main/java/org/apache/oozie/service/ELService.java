 ...
 * <p/>
 * The syntax for a constant definition is <code>PREFIX:NAME=CLASS_NAME#CONSTANT_NAME</code>.
 * <p/>
 * The syntax for a constant definition is <code>PREFIX:NAME=CLASS_NAME#METHOD_NAME</code>.
 */
public class ELService implements Service {
    ...
    private List<ELConstant> constants;
    ...
    public synchronized void init(Services services) throws ServiceException {
        ...
        constants = new ArrayList<ELConstant>();
        constants.addAll(extractConstants(services.getConf(), CONF_CONSTANTS));
        constants.addAll(extractConstants(services.getConf(), CONF_EXT_CONSTANTS));

        functions = new ArrayList<ELFunction>();
        functions.addAll(extractFunctions(services.getConf(), CONF_FUNCTIONS));
        functions.addAll(extractFunctions(services.getConf(), CONF_EXT_FUNCTIONS));
    }
    ...
    /**
     * Return an {@link ELEvaluator} preconfigured with the constants and functions defined in the configuration.
     *
    ...
    public ELEvaluator createEvaluator() {
        ...
        for (ELConstant constant : constants) {
    ...
    private static Method findMethod(String className, String methodName) throws ServiceException {
        Method method = null;
        try {
    ...
    private static Object findConstant(String className, String constantName) throws ServiceException {
        try {
            Class klass = Thread.currentThread().getContextClassLoader().loadClass(className);
