...
public class TestBlockingWritesExitValueProcess {
    ...
    public void testNonBlockingProcess() {
        ...
        assertTrue("Invalid input stream class", process.getInputStream() instanceof BlockingInputStream);
        assertTrue("Invalid error stream class", process.getErrorStream() instanceof BlockingInputStream);
    }

    @Test
    public void testBlockingInputStreamProcess() {
        final BlockingWritesExitValueProcess process = BlockingWritesExitValueProcess.createBufferLimitedProcess(TEST_STRING,
                "", 1);
    ...
    public void testBlockingErrorStreamProcess() {
        final BlockingWritesExitValueProcess process = BlockingWritesExitValueProcess.createBufferLimitedProcess("",
                TEST_STRING, 1);
        expectedException.expect(IllegalThreadStateException.class);
        process.exitValue();
    }
