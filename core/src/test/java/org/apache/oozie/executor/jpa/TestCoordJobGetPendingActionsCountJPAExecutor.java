...
public class TestCoordJobGetPendingActionsCountJPAExecutor extends XDataTestCase {
    Services services;
    ...
    public void testCoordJobGetPendingActionsCount() throws Exception {
        ...
        // Insert 2 coordinator actions with pending true and 1 coordinator action with pending false
        addRecordToCoordActionTable(jobId, actionNum++, CoordinatorAction.Status.FAILED, "coord-action-get.xml", 1);
    ...
    private void _testCoordActionsPendingCount(String jobId, int expectedSize) throws Exception {
        JPAService jpaService = Services.get().get(JPAService.class);
        assertNotNull(jpaService);
        // Call JPAExecutor to get actions which are pending
        CoordJobGetPendingActionsCountJPAExecutor actionGetCmd = new CoordJobGetPendingActionsCountJPAExecutor(jobId);
